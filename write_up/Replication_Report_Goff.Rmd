...
In this paper, Gott et. al. test whether black children enjoy fewer of the basic human protections afforded to their non-black peers. In study 1, the authors simply asked participants about the innocence of children. Participants answered questions about how innocent children were in general (i.e., with-out specifying race) and how innocent White and Black children were.  
...
*See link for https://stanforduniversity.qualtrics.com/jfe/form/SV_3OAAprO9LWTFg1f*
...
This replication will use MTurk paricipants instead of university students. I do not expect this will have a large effect on the results. 
...
```{r}
GoffData = read_csv("../data/Goff Replication - Test 1.csv")
```
...
FinalGoff = GoffData %>%
  gather(QuestionNumber, Response, 2:127) %>%
  mutate(QuestionType = str_extract(QuestionNumber, "protection|need care|care for themselves|danger to others|danger to themselves|cute|innocent")) %>% 
  mutate(Condition = str_extract(QuestionNumber, "Black|White")) %>% 
  mutate(Age = str_extract(QuestionNumber, "0-4|5-9|10-13|14-17|18-21|22-25")) %>%
   na.omit() %>%
...
# Aggregate first.
# this main t.test compares the two conditions, black and white
# use the formula argument for the t-test 
# lhs ~ rhs
# forumala is lh to rh where lh is the value and rh is the groups.
# that is, lhs is the values (the numbers) --> Agemean
# and rhs is the groups that give us the two sets of numbers --> Condition 
# So the formula "Agemean ~ Condition" means that "the agemean that
# participants give *depends* on which condition they are in
# This articulates the hypothesis that we're testing.
# Apparently, the first argument has to be the formula
# and you don't actually type in "formula = " :(
